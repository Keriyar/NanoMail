# NanoMail

<div align="center">

![NanoMail Logo](assets/icons/NanoMail.ico)

**轻量级的 Windows Gmail 通知客户端**

灵感来源于 macOS 版 [Gmail Notification](https://github.com/crayonape/Gmail-Notification)

[![License: GPL v3](https://img.shields.io/badge/License-GPLv3-blue.svg)](https://www.gnu.org/licenses/gpl-3.0)
[![Rust](https://img.shields.io/badge/Rust-2024-orange.svg)](https://www.rust-lang.org/)
[![Platform](https://img.shields.io/badge/Platform-Windows%2010%2F11-blue.svg)](https://www.microsoft.com/windows)

</div>

---

## ✨ 特性

- 🎨 **macOS 风格界面** - 磨砂玻璃效果、圆角设计、流畅动画
- 📧 **实时未读数** - 每 3 分钟自动同步 Gmail 未读邮件数量
- 🔐 **安全认证** - 使用 OAuth2.0 授权,支持 Token 加密存储
- 🖼️ **头像显示** - 自动获取并缓存 Google 账户头像
- 🚀 **轻量高效** - 二进制体积 < 8MB,内存占用 < 30MB
- 🎯 **系统托盘** - 常驻系统托盘
- 🔄 **智能同步** - 自动 Token 刷新,网络断线重连

---

## 🚀 快速开始

### 系统要求

- **操作系统**: Windows 10 (1809+) 或 Windows 11
- **运行时**: 无需额外依赖(静态链接)

### 下载安装

1. 从 [Releases](../../releases) 页面下载最新版本的 `nanomail.exe`
2. 双击运行,程序会自动在系统托盘启动
3. 左键点击托盘图标打开主界面

### 首次使用

1. 点击主界面底部的 **➕ 加号按钮**
2. 浏览器会自动打开 Google 授权页面
3. 登录并授权 NanoMail 访问您的 Gmail
4. 授权成功后自动返回,开始同步未读邮件数

---

## 🎯 功能说明

### 主界面

- **标题栏**:

  - `N` 字母会根据状态变色(🟢 正常 / 🔴 网络错误 / 🟡 Access Token错误)
  - 🌙 **月亮图标**: 切换主题(暂未实现)
  - ✉️ **信封图标**: 在浏览器中打开 Gmail

- **账户卡片**:

  - 显示账户名、邮箱地址、头像
  - 未读邮件数徽章(绿色: 正常 / 红色: 错误)

- **操作栏**:
  - ➕ **加号**: 添加新账户
  - 🐛 **Bug**: 反馈问题(打开 GitHub Issues)
  - ⬅️ **箭头**: 隐藏到托盘

### 系统托盘

- **左键点击**: 显示/隐藏主窗口
- **右键菜单**:
  - 📧 显示 - 打开主界面
  - ❌ 退出 - 关闭应用

### 状态指示

托盘图标和标题 `N` 字母会根据应用状态变色:

| 状态        | 颜色 | 说明                  |
| ----------- | ---- | --------------------- |
| 🟢 正常     | 绿色 | 所有账户同步正常      |
| 🔴 有未读   | 红色 | Token 过期或 API 错误 |
| 🟡 网络错误 | 黄色 | 网络不可用            |

---

## 🔧 开发指南

### 环境准备

1. **安装 Rust**:

   ```bash
   # 从 https://rustup.rs/ 下载安装
   rustup default stable
   ```

2. **克隆仓库**:
   ```bash
   fork to your account
   git clone https://github.com/yourusername/nanomail.git
   cd nanomail
   ```

### 开发命令

```bash
# 开发构建
cargo build              # 构建 debug 版本
cargo run                # 运行(会在系统托盘启动)
cargo check              # 快速语法检查

# 代码质量
cargo fmt                # 格式化代码
cargo clippy             # 运行 linter

# 测试
cargo test               # 运行所有测试
cargo test --lib         # 只运行单元测试

# 发布构建
cargo build --release    # 构建优化版本
```

### 配置 OAuth2

要使用 Gmail API,需要在 Google Cloud Console 配置:

1. 访问 [Google Cloud Console](https://console.cloud.google.com/)
2. 创建新项目或选择现有项目
3. 启用 **Gmail API** 和 **Google+ API**
4. 创建 **OAuth 2.0 客户端 ID**(桌面应用)
5. 配置环境变量(系统)：`GMAIL_CLIENT_ID`, `GMAIL_CLIENT_SECRET`
6. 添加测试用户(开发阶段)

---

## 📁 项目结构

```
NanoMail/
├── src/
│   ├── main.rs              # 主入口:初始化日志、UI、托盘、同步引擎
│   ├── config/              # 配置管理(OAuth、账户存储、加密)
│   ├── mail/gmail/          # Gmail 模块(OAuth、Token、API)
│   ├── sync/                # 同步引擎(定时同步)
│   ├── tray/                # 系统托盘(图标、菜单、事件)
│   ├── ui/                  # UI 桥接
│   └── utils/               # 工具库(HTTP 客户端、机器指纹)
├── ui/                      # Slint UI 定义
│   ├── main.slint           # 主窗口
│   ├── themes/colors.slint  # 颜色主题
│   └── components/          # UI 组件
├── assets/                  # 资源文件
│   └── icons/               # 图标和 SVG
├── build.rs                 # 构建脚本
└── Cargo.toml               # 项目配置
```

---

## 🛠️ 技术栈

| 技术          | 版本 | 用途            |
| ------------- | ---- | --------------- |
| **Rust**      | 2024 | 核心语言        |
| **Slint**     | 1.8  | 声明式 GUI 框架 |
| **Tokio**     | 1.38 | 异步运行时      |
| **reqwest**   | 0.12 | HTTP 客户端     |
| **oauth2**    | 4.4  | OAuth2 认证     |
| **tray-icon** | 0.14 | 系统托盘        |
| **aes-gcm**   | 0.10 | Token 加密      |
| **argon2**    | 0.5  | 密钥派生        |

---

## 🔐 隐私与安全

- ✅ 使用 OAuth2.0 授权,**不存储密码**
- ✅ Access Token 和 Refresh Token 使用 **AES-GCM 加密**存储
- ✅ 加密密钥基于**机器指纹**派生,防止跨设备窃取
- ✅ 所有 API 调用使用 **HTTPS** 加密传输
- ✅ 账户数据存储在 `%APPDATA%/NanoMail/accounts/`(仅本地)
- ✅ **开源透明**,代码可审计

---

## 🤝 贡献指南

欢迎提交 Issue 和 Pull Request!

### 开发流程

1. Fork 本仓库
2. 创建特性分支: `git checkout -b feature/AmazingFeature`
3. 提交更改: `git commit -m 'feat: Add some AmazingFeature'`
4. 推送分支: `git push origin feature/AmazingFeature`
5. 提交 Pull Request

### 代码规范

- 遵循 Rust 官方代码风格(`cargo fmt`)
- 运行 Clippy 检查(`cargo clippy`)
- 添加必要的单元测试
- 更新相关文档

---

## 📄 许可证

本项目采用 [GPL-3.0 License](LICENSE) 开源协议。

**注意**: 本项目仅用于学术目的(毕业设计项目)。

---

## 🙏 致谢

- 灵感来源: [Gmail Notification (macOS)](https://github.com/crayonape/Gmail-Notification)
- UI 框架: [Slint](https://slint.dev/)
- 图标设计: [Icofront-阿里巴巴矢量图标库](https://www.iconfont.cn/)

---

## 📧 联系方式

- **Issues**: [GitHub Issues](../../issues)
- **Discussions**: [GitHub Discussions](../../discussions)

---

<div align="center">

**如果觉得这个项目有帮助,请给个 ⭐ Star 支持一下!**

Made with ❤️ by NanoMail Project

</div>
